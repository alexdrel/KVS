...
func (b *Binder) declareSymbolAndAddToSymbolTable(node *ast.Node, symbolFlags ast.SymbolFlags, symbolExcludes ast.SymbolFlags) *ast.Symbol {
	...
	case ast.KindEnumDeclaration:
		return b.declareSymbol(ast.GetExports(b.container.Symbol()), b.container.Symbol(), node, symbolFlags, symbolExcludes)
	case ast.KindTypeLiteral, ast.KindObjectLiteralExpression, ast.KindKvsCompactObjectExpression, ast.KindKvsTypedObjectExpression, ast.KindInterfaceDeclaration, ast.KindJsxAttributes:
		return b.declareSymbol(ast.GetMembers(b.container.Symbol()), b.container.Symbol(), node, symbolFlags, symbolExcludes)
...
func (b *Binder) bindChildren(node *ast.Node) {
	...
	case ast.KindObjectLiteralExpression, ast.KindKvsCompactObjectExpression, ast.KindKvsTypedObjectExpression, ast.KindArrayLiteralExpression, ast.KindPropertyAssignment, ast.KindSpreadElement:
		b.inAssignmentPattern = saveInAssignmentPattern
		b.bindEachChild(node)
	default:
		b.bindEachChild(node)
...
func (b *Binder) bindKvsIfBindingStatement(node *ast.Node) {
	...
	clause.Condition = conditionExpression
	b.bindCondition(conditionExpression, thenLabel, elseLabel)
	b.currentFlow = b.finishFlowLabel(thenLabel)
...
func GetContainerFlags(node *ast.Node) ContainerFlags {
	switch node.Kind {
	case ast.KindClassExpression, ast.KindClassDeclaration, ast.KindEnumDeclaration, ast.KindObjectLiteralExpression, ast.KindKvsCompactObjectExpression, ast.KindKvsTypedObjectExpression, ast.KindTypeLiteral,
